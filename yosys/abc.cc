...
void abc_module(RTLIL::Design *design, RTLIL::Module *current_module, std::string script_file, std::string exe_file,
		...
		const std::vector<RTLIL::Cell*> &cells, bool show_tempdir, bool sop_mode, bool abc_dress , bool my_work, bool abc_also, bool abc_aiger, bool abc_dm )
{
	...
	if (abc_dress)
		abc_script += "; dress";
	// if(my_work){
	// 	abc_script = stringf("; strash; print_stats; write %s/abc_output.aig", tempdir_name.c_str());
	// }
    if (abc_also)
    	if (abc_aiger || abc_dm)
      		abc_script += stringf("; strash; print_stats; write %s/abc_output.aig", tempdir_name.c_str());
    	else
      		abc_script += stringf("; write_blif -j %s/abc_output.blif; strash; print_stats", tempdir_name.c_str());
	else
		// abc_script += stringf("; print_stats; write_blif  %s/output.blif; strash; print_stats", tempdir_name.c_str());
		abc_script += stringf("; print_stats; write_blif  %s/output.blif;", tempdir_name.c_str());
	...
	if (count_output > 0)
	{
		...
		if(my_work){
			...
			abc_script += stringf(" source /home/slt/my_work/scripts/abc.rc; ps; resyn; compress2; share; strash; ifraig; dc2; if -g -K 6; resyn2; ps; write_aiger %s/abc_bef.aig", tempdir_name.c_str());
			buffer = stringf("abc -c \"%s\"",abc_script.c_str());
			log("Running ABC command: %s\n", replace_tempdir(buffer, tempdir_name, show_tempdir).c_str());
			// buffer = stringf("%s -s -f /home/slt/my_work/scripts/abc_map_bef.script", exe_file.c_str());			
			ret = run_command(buffer);
			log("Running LSOracle command");
			std::string lsoracle_script = stringf("lsoracle -c \"read_aig %s/abc_bef.aig; oracle; write_blif -g %s/oracle.blif\"",tempdir_name.c_str(),tempdir_name.c_str());
			buffer = lsoracle_script;
			ret = run_command(buffer);
			log("Running ABC mapping command");
			buffer = stringf("abc -c \"read_blif %s/oracle.blif; source /home/slt/my_work/scripts/abc.rc; read /home/slt/my_work/scripts/mcnc.genlib; ps; &get -n; &syn2; &dch -f; &nf -F 10 -A 10; &put; ps; write_blif %s/output.blif\"",tempdir_name.c_str(),tempdir_name.c_str());		
			//buffer = stringf("abc -c \"read_blif %s/oracle.blif; source /home/slt/my_work/scripts/abc.rc; read /home/slt/my_work/scripts/stdcells.genlib; ps; strash; &get -n; &syn2; &dch -f; &nf -F 10 -A 10; &put; ps; write_blif %s/output.blif\"",tempdir_name.c_str(),tempdir_name.c_str());		
			abc_output_filter filt(tempdir_name, show_tempdir);
			ret = run_command(buffer, std::bind(&abc_output_filter::next_line, filt, std::placeholders::_1));
		}else{
			buffer = stringf("%s -s -f %s/abc.script 2>&1", exe_file.c_str(), tempdir_name.c_str());
			// buffer = stringf("abc -c \"read_blif output.blif; &get; &nf; &put; write_blif\"");
			log("Running ABC command: %s\n", replace_tempdir(buffer, tempdir_name, show_tempdir).c_str());
			abc_output_filter filt(tempdir_name, show_tempdir);
...
// #endif
      ...
      if (abc_aiger)
      {
        buffer = stringf("also -c \"read_aiger %s/abc_output.aig; exact_map; ps -x; xmgrs; xmgrw; xmgrw; ps -x; restore_names -a -f %s/abc_output.aig -o %s/also_output.blif\"",tempdir_name.c_str(), tempdir_name.c_str(), tempdir_name.c_str());
        printf( "ALSO command: %s\n", buffer.c_str());
      }
      else if (abc_dm)
      {
        buffer = stringf("also -c \"read_aiger %s/abc_output.aig; dm --aig_to_xmg; ps -x; xmgrs; xmgrw; xmgrw; ps -x; restore_names -a -f %s/abc_output.aig -o %s/also_output.blif\"",tempdir_name.c_str(), tempdir_name.c_str(), tempdir_name.c_str());
        printf( "ALSO command: %s\n", buffer.c_str());
      }
      else
      {
        buffer = stringf("also -c \"read_blif %s/abc_output.blif; lut_resyn -nx; ps -x; xmgrs; xmgrw; xmgrw; ps -x; restore_names -f %s/abc_output.blif -o %s/also_output.blif\"",tempdir_name.c_str(), tempdir_name.c_str(), tempdir_name.c_str());
        printf( "ALSO command: %s\n", buffer.c_str());
      }
			
      ret = run_command( buffer );
			buffer = stringf("abc -c \"read_blif %s/also_output.blif; read_lib -w %s; &get -n; &dch -f; &nf; &put; print_stats; write_blif %s/output.blif\"",tempdir_name.c_str(), liberty_files[0].c_str(), tempdir_name.c_str());
			ret = run_command( buffer );
		}
		buffer = stringf("%s/%s", tempdir_name.c_str(), "output.blif");
		//buffer = stringf("%s", "test.blif");
		std::ifstream ifs;
		ifs.open(buffer);
		if (ifs.fail())
		...
		if(abc_also||my_work){
			mapped_mod = mapped_design->module(ID(top));
		}
...
struct AbcPass : public Pass {
	...
	void help() override
	{
		...
		log("\n");
		log("    -my_work\n");
		log("        run the 'abc|lsoracle|also' command.\n ");
		log("\n");
		log("    -also\n");
		log("        run the 'also' command after all other ABC commands.\n ");
		log("\n");
		log("    -aiger\n");
		log("        run the 'also' command after all other ABC commands, use AIGER format as the ALSO input.\n ");
		log("\n");
		log("    -dm\n");
		log("        run the 'also' command after all other ABC commands, use AIGER format as the ALSO input and direct mapping from AIG to XMG.\n ");
		log("\n");
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		abc_also = design->scratchpad_get_bool("abc.also", abc_also);
		abc_aiger = design->scratchpad_get_bool("abc.aiger", abc_aiger);
		abc_dm = design->scratchpad_get_bool("abc.dm", abc_dm);
		...
		for (auto mod : design->selected_modules())
		{
			...
			if (!dff_mode || !clk_str.empty()) {
				abc_module(design, mod, script_file, exe_file, liberty_files, constr_file, cleanup, lut_costs, dff_mode, clk_str, keepff,
						delay_target, sop_inputs, sop_products, lutin_shared, fast_mode, mod->selected_cells(), show_tempdir, sop_mode, abc_dress, my_work, abc_also, abc_aiger, abc_dm);
				continue;
			}
			...
			for (auto &it : assigned_cells) {
				...
				abc_module(design, mod, script_file, exe_file, liberty_files, constr_file, cleanup, lut_costs, !clk_sig.empty(), "$",
						keepff, delay_target, sop_inputs, sop_products, lutin_shared, fast_mode, it.second, show_tempdir, sop_mode, abc_dress, my_work, abc_also, abc_aiger, abc_dm);
				assign_map.set(mod);
			}
		}
